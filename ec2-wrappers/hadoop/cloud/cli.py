...
DEFAULT_CLOUD_PROVIDER = 'cs61cec2'
...
PROXY_PORT_OPTION = \
  make_option("--proxy-port", metavar="PORT", default=6666,
    help="The port for the SOCKS proxy.")
...
providers only.)"),
  make_option("--use-spot", action="store_true", default=False,
...
def check_launch_options_set(cluster, options):
  if cluster.get_provider_code() == 'ec2' or cluster.get_provider_code() == 'cs61cec2':
