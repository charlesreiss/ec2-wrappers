...
our %defaults = (
    start_servers          => 8,      # start this many, and don't go below
    max_clients            => 16,     # don't go above
    max_requests_per_child => 250,    # just in case there's a leak
    min_spare_servers      => 4,      # minimum idle (if 0, never start new)
    max_spare_servers => 12,    # maximum idle (should be "single browser max")
    verify_delay      => 30,    # minimum time between kids verification
);
